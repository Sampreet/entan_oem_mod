...
def func_mdy_max(system_params, val, logger, results):
    # system parameters
    system_params['n_ths'][0] = 1 / (np.exp(sc.hbar * 2 * np.pi * 1e6 / sc.k / system_params['T_b']) - 1)
    # update parameters
...
looper = wrap_looper(SystemClass=OEM_20, params=params, func=func_mdy_max, looper='XLooper', file_path_prefix='data/v2.0-qom-v0.8.5/r_1a_En_0')
X = looper.axes['X']['val']
...
looper = wrap_looper(SystemClass=OEM_20, params=params, func=func_mdy_max, looper='XLooper', file_path_prefix='data/v2.0-qom-v0.8.5/r_1a_En_1')
