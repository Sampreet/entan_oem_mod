...
params = {
    ...
    'plotter': {
        ...
        'x_label': '$T_{c}$ (K)',
        'x_label_pad': 20,
        'x_tick_labels': ['$10^{' + str(i - 7) + '}$' for i in range(5)],
        'x_tick_pad': 16,
        'x_ticks': [10**(i - 7) for i in range(5)],
        'x_ticks_minor': [(i % 10 + 1) * 10**(int(i / 10) - 7) for i in range(41)],
        'x_scale': 'log',
        'y_colors': ['b', 'b'],
        'y_sizes': [2, 2],
        'y_styles': [':', '-'],
...
def func_mdy_max(system_params, val, logger, results):
    # system parameters
    system_params['n_ths'][1] = 1 / (np.exp(sc.hbar * 2 * np.pi * 1.1e6 / sc.k / system_params['T_c']) - 1)
    # update parameters
...
looper = wrap_looper(SystemClass=OEM_20, params=params, func=func_mdy_max, looper='XLooper', file_path_prefix='data/v2.0-qom-v0.8.5/r_1b_En_0')
X = looper.axes['X']['val']
...
looper = wrap_looper(SystemClass=OEM_20, params=params, func=func_mdy_max, looper='XLooper', file_path_prefix='data/v2.0-qom-v0.8.5/r_1b_En_1')
